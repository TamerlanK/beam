...
export function initToasts() {
  ...
  on("toast", ({ msg, kind }) => {
    push(el("div", { class: `toast ${kind}`, role: kind === "bad" ? "alert" : "status" },
      el("span", { class: "toast-ic" }, icon(GLYPH[kind] || "info")),
      el("span", { text: msg })), kind === "bad" ? 5500 : 3800);
  });

  // Notes stay until dismissed: a one-time code shouldn't vanish while you're typing it.
  on("snippet", (d) => {
    const url = asURL(d.text);
    const t = el("div", { class: "toast note", role: "status" },
      el("b", { class: "note-from", text: `Note from ${d.from ? d.from.name : "someone"}` }),
      el("pre", { class: "snip-recv", text: d.text, tabindex: "0" }),
      el("div", { class: "note-act" },
        url ? el("a", { class: "btn btn-ghost", href: url, target: "_blank", rel: "noopener noreferrer" }, "Open link") : null,
        el("button", { class: "btn btn-ghost", type: "button", onclick: async () => { await copyText(d.text); toast("Copied", "ok"); } }, "Copy"),
